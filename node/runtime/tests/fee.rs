...
fn charge_extrinsic_fee_for_generic_asset_transfer_overflow() {
	with_externalities(
		...
		|| {
			...
			System::set_extrinsic_index(0);
			assert_err!(ExtrinsicFeePayment::charge_extrinsic_fee(&DEFAULT_TRANSACTOR, 7, &xt), "extrinsic fee overflow (base + bytes + call)");
	});
}
...
fn charge_extrinsic_fee_fails_with_bytes_fee_overflow() {
	...
	with_externalities(
		...
		|| {
			System::set_extrinsic_index(0);
			assert_err!(ExtrinsicFeePayment::charge_extrinsic_fee(&DEFAULT_TRANSACTOR, 2, &xt), "extrinsic fee overflow (bytes)");
		},
	);
}
...
fn charge_extrinsic_fee_fails_with_total_fee_overflow() {
	...
	with_externalities(
		...
		|| {
			System::set_extrinsic_index(0);
			assert_err!(ExtrinsicFeePayment::charge_extrinsic_fee(&DEFAULT_TRANSACTOR, 1, &xt), "extrinsic fee overflow (base + bytes)");
		},
	);
}
