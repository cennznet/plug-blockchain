...
impl Peerset {
	...
	fn on_remove_reserved_peer(&mut self, peer_id: PeerId) {
		...
		info!(target: "peerset", "Removing reserved peer {:?} with reputation {}", &peer_id, reputation);
	...
	fn on_report_peer(&mut self, peer_id: PeerId, score_diff: i32) {
		...
		if score < 0 {
			if !is_reserved_node {
				...
				info!(target: "peerset", "Disconnecting from {:?} because reputation {} is below 0", &peer_id, score);
