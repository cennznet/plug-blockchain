...
impl<Block: BlockT> Inner<Block> {
    ...
    fn validate_round_message(
        &self,
        who: &PeerId,
        full: &VoteOrPrecommitMessage<Block>,
    ) -> Action<Block::Hash> {
...
impl<Block: BlockT> GossipValidator<Block> {
        ...
        F: FnOnce(Vec<PeerId>, NeighborPacket<NumberFor<Block>>),
    {
        self.inner
            .write()
            .note_commit_finalized(finalized, send_neighbor);
    }
    ...
    pub(super) fn do_validate(
        ...
        let action = {
            ...
            match GossipMessage::<Block>::decode(&mut data) {
                ...
                Some(GossipMessage::Commit(ref message)) => {
                    inner.validate_commit_message(who, message)
                }
                Some(GossipMessage::Neighbor(update)) => {
                    let (topics, action) =
                        inner.import_neighbor_message(who, update.into_neighbor_packet());
