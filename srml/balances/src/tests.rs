...
fn dust_account_removal_should_work() {
	with_externalities(
		...
		|| {
			...
			assert_ok!(Balances::transfer((Some(2), None).into(), 5, 1901)); // index 1 (account 2) becomes zombie
			assert_eq!(Balances::total_balance(&2), 0);
...
fn dust_account_removal_should_work2() {
	with_externalities(
		...
		|| {
			...
			assert_ok!(Balances::transfer((Some(2), None).into(), 5, 1851));
			assert_eq!(Balances::total_balance(&2), 0);
...
fn balance_transfer_when_reserved_should_not_work() {
	with_externalities(&mut ExtBuilder::default().build(), || {
		let _ = Balances::deposit_creating(&1, 111);
		assert_ok!(Balances::reserve(&1, 69));
		assert_noop!(Balances::transfer((Some(1), None).into(), 2, 69), "balance too low to send value");
	});
}
...
fn transfer_overflow_isnt_exploitable() {
	with_externalities(
		...
		|| {
			...
			assert_err!(
				Balances::transfer((Some(1), None).into(), 5, evil_value),
